...
function App() {
  ...
  const obtenerVentas = async () => {
    try {
      const respuesta = await axios.get("http://localhost:8080/ventas");
  ...
  const obtenerInventario = async () => {
    try {
      const respuesta = await axios.get("http://localhost:8080/inventario");
  ...
  const crearVenta = async () => {
    try {
      await axios.post("http://localhost:8080/ventas", nuevaVenta);
  ...
  const actualizarStock = async (id, stockActual) => {
    try {
      ...
      await axios.put(
        `http://localhost:8080/inventario/${id}/stock`,
        {
          stock: nuevoStock,
        }
      );
